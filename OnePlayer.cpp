...
void COnePlayer::OnRButtonDown(UINT nFlags, CPoint point)
{
	...
	if (showPlayInfo)//已经显示状态栏
	{
		menu.GetSubMenu(0)->ModifyMenu(ID_Menu_ShowPlayInfo,MF_BYCOMMAND ,ID_MENU_HidePlayInfo,_T("隐藏状态栏"));
		
	}
	else
	{
		menu.GetSubMenu(0)->ModifyMenu(ID_MENU_HidePlayInfo,MF_BYCOMMAND ,ID_Menu_ShowPlayInfo,_T("显示状态栏"));
